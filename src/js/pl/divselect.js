define(
function(){
function DivSelectYHY(id, divId, className) {{
	this.id = id;
	this.divId = divId;
	...
	for (var i = 0; i < this.ele.options.length; i++) {
		...
		temp.onclick = function() {
            ...
            if (document.dispatchEvent){
                ...
                that.ele.dispatchEvent(event1);
            }else{
            	 that.ele.fireEvent("onchange");
            }
...
DivSelectYHY.prototype = {
	...
	open: function() {
		...
		this.parentObj.style.zIndex="9999";
	},
	close: function() {
		var that = this;
		this.__div_list.style.display = "none";
